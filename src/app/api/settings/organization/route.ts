...
import {
  ...
  materialCategories, inventoryPool, users, organizations,
} from "@/lib/db/schema";
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    if (confirmation !== "DELETE ORGANIZATION") {
      return NextResponse.json(
        { error: "Confirmation text must exactly match 'DELETE ORGANIZATION'" },
        { status: 400 },
      );
    }
    ...
    await db.transaction(async (tx) => {
      ...
      await tx.delete(users).where(eq(users.organization_id, orgId));
    ...
    logActivity({
      ...
      entityId: orgId,
      description: "Deleted entire organization",
    });

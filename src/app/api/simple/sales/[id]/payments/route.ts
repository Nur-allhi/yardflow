...
const simpleSalePaymentSchema = z.object({
...
export async function POST(
  ...
  try {
    ...
    const parsed = simpleSalePaymentSchema.safeParse(body);
    if (!parsed.success) {
    ...
    await logActivity({
      ...
      action: "payment",
      entityType: "simple_sale_payment",
      entityId: result.id,
      description: `Recorded payment of ${parsed.data.amount} for simple sale`,
    });

          ...
          <div className="bg-white border border-[#c6c6cd]/50 rounded-xl shadow-sm overflow-hidden">
            ...
            <div className="p-5 md:p-6 grid grid-cols-2 gap-y-5">
              <div>
                ...
                <p className="font-medium text-[#0F172A] text-sm">{vendor.phone || "—"}</p>
              </div>
              <div>
                <p className="text-[10px] uppercase font-bold text-[#505f76] tracking-wider mb-1">Address</p>
                <p className="font-medium text-[#0F172A] text-sm">{vendor.address || "—"}</p>
              </div>
            </div>
          </div>
                    ...
                    {vendor.purchases.map((p) => (
                      <tr key={p.id} className="hover:bg-[#F8FAFC]">
                        <td className="px-6 py-4">
                          <Link href={`/purchases/${p.id}`} className="font-medium text-sm text-[#0F172A] hover:text-[#059669] hover:underline">
                        ...
                        <td className={`px-6 py-4 font-mono text-sm text-right font-bold ${p.due_amount > 0 ? "text-[#CA8A04]" : "text-[#16A34A]"}`}>
          ...
          <div className="bg-[#0F172A] text-white rounded-xl p-6 shadow-md relative overflow-hidden">
            ...
            <div className="space-y-4 relative z-10">
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="font-mono font-bold text-[#059669]">{formatMoney(vendor.summary.total_paid)}</span>
              </div>
              <div className="flex justify-between items-center border-t border-white/20 pt-4">
                <span className="text-white font-bold text-sm">Total Due</span>
                <span className={`font-mono font-bold text-lg ${vendor.summary.total_due > 0 ? "text-[#EAB308]" : "text-[#059669]"}`}>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-[#0F172A]/40 backdrop-blur-sm p-4">
          ...
          <div className="bg-white w-full max-w-md rounded-xl shadow-lg border border-[#c6c6cd]/30 relative z-10 overflow-hidden">
            <div className="flex items-center justify-between p-6 pb-0">
              <h2 className="font-display font-bold text-xl text-[#0F172A]">Record Payment</h2>
            ...
            <div className="mx-6 mt-4 p-4 bg-[#EAB308]/10 border border-[#EAB308]/20 rounded-lg">
              <div className="flex justify-between items-center">
                ...
                <span className="font-mono font-bold text-[#EAB308] text-lg">{formatMoney(vendor.summary.total_due)}</span>
              </div>
            ...
            <form onSubmit={handlePayment} className="p-6 space-y-5">
              <div className="space-y-1.5">
                <label className="text-xs font-bold uppercase tracking-wider text-[#505f76]">
                  Pay Against Purchase <span className="text-[#ba1a1a]">*</span>
                </label>
                <select
                  value={payPurchaseId}
                  onChange={(e) => {
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-bold uppercase tracking-wider text-[#505f76]">Pay From Account</label>
                <select
                  value={payAccountId}
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-bold uppercase tracking-wider text-[#505f76]">Payment Date</label>
                <input
                  type="date"
                  value={payDate}

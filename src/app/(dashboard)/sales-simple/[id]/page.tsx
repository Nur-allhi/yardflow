...
function StatusChip({ status }: { status: string }) {
  ...
  return <span className={`px-2 py-1 ${s.bg} ${s.text} text-[10px] font-bold rounded uppercase tracking-wider`}>{s.label}</span>;
}

function TypeChip({ type, isQuickCash }: { type: string | null; isQuickCash: boolean }) {
  if (isQuickCash) {
    return <span className="text-[10px] font-bold px-2 py-0.5 rounded border border-tertiary text-tertiary bg-tertiary/5 uppercase">Quick Cash</span>;
  }
  ...
  return <span className={`text-[10px] font-bold px-2 py-0.5 rounded border ${s.border} ${s.text} ${s.bg} uppercase`}>{s.label}</span>;
}

export default function SimpleSaleDetailPage() {
  ...
  const { data: sale, isLoading, error } = useQuery<SimpleSaleDetail>({
    queryKey: ["simple-sale", id],
    queryFn: async () => {
      const res = await fetch(`/api/simple/sales/${id}`);
  ...
  const paymentMutation = useMutation({
    mutationFn: async (data: { amount: number; account_id: string; payment_date: string; note?: string }) => {
      const res = await fetch(`/api/simple/sales/${id}/payments`, {
        method: "POST",
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["simple-sale", id] });
  ...
  const deleteMutation = useMutation({
    mutationFn: async () => {
      const res = await fetch(`/api/simple/sales/${id}`, { method: "DELETE" });
      if (!res.ok) {
        const err = await res.json();
  ...
  function openPaymentModal() {
    ...
    setPayAmount(sale ? String(Math.ceil(sale.due_amount)) : "");
    setPayError(null);
    ...
    return (
      <div className="p-4 md:p-8">
        <div className="bg-red-50 border border-red-200 rounded-xl p-8 text-center">
          <p className="text-error font-medium text-lg mb-2">{error instanceof Error ? error.message : "Sale not found"}</p>
          <Link href="/sales-simple" className="inline-flex items-center gap-2 px-4 py-2 bg-primary-container text-white text-sm rounded-lg">
  ...
  const paidPercent = sale.total_amount > 0 ? ((sale.paid_amount / sale.total_amount) * 100).toFixed(1) : "0";

  return (
    <div className="p-4 md:p-8">
      <Breadcrumb items={[{ label: "Dashboard", href: "/" }, { label: "Simple Sales", href: "/sales-simple" }, { label: `Sale #${id.slice(0, 4).toUpperCase()}`, href: null }]} />
      ...
      <div className="flex flex-col md:flex-row md:items-center gap-3 mb-6">
        ...
        <div className="md:ml-auto flex gap-2">
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 border border-error text-error font-bold text-sm rounded-lg hover:bg-error/5 transition-all active:scale-95"
          >
      ...
      <div className="md:hidden bg-surface-container-lowest rounded-xl p-5 shadow-sm border border-outline-variant mb-4">
        <div className="flex justify-between items-start mb-4">
          ...
          <div className="text-right">
            ...
            <p className={`font-headline font-bold text-lg font-mono tracking-tight ${sale.due_amount > 0 ? "text-warning" : "text-success"}`}>{formatMoney(sale.due_amount)}</p>
          </div>
        </div>
        ...
        <div className="lg:col-span-7 space-y-6">
          <div className="bg-white border border-outline-variant/50 rounded-xl shadow-sm overflow-hidden">
            ...
            <div className="p-5 md:p-6 grid grid-cols-2 gap-y-5">
              <div>
                <p className="text-[10px] uppercase font-bold text-secondary tracking-wider mb-1">Customer</p>
                {sale.is_quick_cash_sale ? (
                  <span className="italic text-secondary text-sm">{sale.customer_name || "Cash Sale"}</span>
                ) : (
                  <p className="font-medium text-primary-container text-sm">{sale.customer_name || "—"}</p>
                )}
              </div>
              <div>
                <p className="text-[10px] uppercase font-bold text-secondary tracking-wider mb-1">Sale Type</p>
                <TypeChip type={sale.sale_type} isQuickCash={sale.is_quick_cash_sale} />
              </div>
              <div>
                <p className="text-[10px] uppercase font-bold text-secondary tracking-wider mb-1">Sale Date</p>
                <p className="font-medium text-primary-container text-sm">{formatDate(sale.sale_date)}</p>
              </div>
              {sale.note && (
                <div className="col-span-2">
                  <p className="text-[10px] uppercase font-bold text-secondary tracking-wider mb-1">Note</p>
                  <p className="text-sm text-primary-container italic">&ldquo;{sale.note}&rdquo;</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="bg-white border border-outline-variant/50 rounded-xl shadow-sm overflow-hidden">
              ...
              <table className="w-full text-left border-collapse">
                <thead className="bg-surface-container-high border-b border-outline-variant">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-[10px] font-bold uppercase text-secondary tracking-wider text-right">Qty (kg)</th>
                    <th className="px-6 py-3 text-[10px] font-bold uppercase text-secondary tracking-wider text-right">Price/kg</th>
                ...
                <tbody className="divide-y divide-outline-variant/30">
                  {sale.items.map((item) => (
                    <tr key={item.id} className="hover:bg-background">
                      <td className="px-6 py-4 text-sm font-bold text-primary-container">{item.description || "—"}</td>
                      <td className="px-6 py-4 font-mono text-sm text-right">{item.quantity_kg.toFixed(3)}</td>
                      <td className="px-6 py-4 font-mono text-sm text-right">{formatMoney(item.price_per_kg)}</td>
                      <td className="px-6 py-4 font-mono text-sm text-right font-semibold">{formatMoney(item.total_amount)}</td>
                    </tr>
                  ))}
                </tbody>
                <tfoot>
                  <tr className="bg-surface-container-low border-t-2 border-primary-container/10">
                    <td colSpan={3} className="px-6 py-4 font-display font-bold text-primary-container text-right uppercase tracking-widest text-xs">Total Amount</td>
                    <td className="px-6 py-4 font-mono text-right text-lg font-bold text-primary-container">{formatMoney(sale.total_amount)}</td>
                  </tr>
                </tfoot>
              </table>
            </div>
            <div className="md:hidden">
              {sale.items.map((item, idx) => (
                <div key={item.id} className={`p-4 ${idx < sale.items.length - 1 ? "border-b border-outline-variant" : ""}`}>
                  <div className="flex justify-between items-center mb-2">
                    <h4 className="font-headline font-bold text-sm text-primary">{item.description || "—"}</h4>
                    <span className="font-mono font-bold text-primary">{formatMoney(item.total_amount)}</span>
                  </div>
          ...
          <div className="bg-primary-container text-white rounded-xl p-6 shadow-md relative overflow-hidden">
            ...
            <div className="space-y-4 relative z-10">
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="font-mono font-bold text-tertiary">{formatMoney(sale.paid_amount)}</span>
              </div>
              <div className="flex justify-between items-center border-t border-white/20 pt-4">
                <span className="text-white font-bold text-sm">Balance Due</span>
                <span className={`font-mono font-bold text-lg ${sale.due_amount > 0 ? "text-warning" : "text-tertiary"}`}>{formatMoney(sale.due_amount)}</span>
              </div>
            </div>
            <div className="mt-6">
              ...
              <div className="h-3 w-full bg-white/20 rounded-full overflow-hidden flex">
                <div className="h-full bg-tertiary rounded-full transition-all duration-500" style={{ width: `${paidPercent}%` }} />
              </div>
            </div>
          </div>
                ...
                {sale.payments.map((pmt) => (
                  <div key={pmt.id} className="p-5 md:p-6 flex items-start gap-4 bg-tertiary/5">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-secondary"><span className="font-bold">Account:</span> {pmt.account_name || "—"}</p>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-primary-container/40 backdrop-blur-sm p-4">
          ...
          <div className="bg-white w-full max-w-md rounded-xl shadow-lg border border-outline-variant/30 relative z-10 overflow-hidden">
            ...
            <div className="mx-6 mt-4 p-4 bg-warning/10 border border-warning/20 rounded-lg">
              <div className="flex justify-between items-center">
                <span className="text-sm font-medium text-secondary">Outstanding Due</span>
                <span className="font-mono font-bold text-warning text-lg">{formatMoney(sale.due_amount)}</span>
              </div>
            </div>
            <form onSubmit={handlePayment} className="p-6 space-y-5">
              ...
              <div className="space-y-1.5">
                ...
                <input type="date" value={payDate} onChange={(e) => setPayDate(e.target.value)} required className="w-full h-[42px] border border-outline-variant rounded bg-white px-3 text-sm focus:border-primary-container focus:ring-0 outline-none" />
              </div>
              <div className="space-y-1.5">
                <label className="text-xs font-bold uppercase tracking-wider text-secondary">Note (optional)</label>
                <textarea value={payNote} onChange={(e) => setPayNote(e.target.value)} rows={2} className="w-full border border-outline-variant rounded bg-white p-3 text-sm focus:border-primary-container focus:ring-0 outline-none resize-none" placeholder="Additional note..." />
              </div>
              <div className="bg-surface-container-low rounded-lg p-4 space-y-2 border border-outline-variant/50">
                ...
                <div className="flex justify-between items-center text-xs">
                  ...
                  <span className="font-mono text-tertiary font-bold">-{formatMoney(parseFloat(payAmount) || 0)}</span>
                </div>
                <div className="flex justify-between items-center pt-2 border-t border-outline-variant/50">
                  ...
                  <span className="font-mono font-bold text-warning">{formatMoney(sale.due_amount - (parseFloat(payAmount) || 0))}</span>
                </div>
              </div>
              {payError && <p className="text-sm text-error font-medium">{payError}</p>}
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={() => setShowPaymentModal(false)} className="flex-1 h-[42px] bg-transparent text-secondary hover:bg-surface-container-low transition-colors font-bold text-sm rounded">Cancel</button>
                <button type="submit" disabled={paymentMutation.isPending} className="flex-1 h-[42px] bg-primary-container text-white hover:bg-primary-container/90 transition-all active:scale-95 font-bold text-sm rounded shadow-md disabled:opacity-40">
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-primary-container/40 backdrop-blur-sm p-4">
          ...
          <div className="bg-white w-full max-w-sm rounded-xl shadow-lg border border-outline-variant/30 relative z-10 p-6">
            <div className="text-center">
              <span className="material-symbols-outlined text-5xl text-error block mb-4">warning</span>
              <h2 className="font-display font-bold text-xl text-primary-container mb-2">Delete Sale?</h2>
              <p className="text-secondary text-sm mb-6">This will permanently delete this sale and restore inventory. This action cannot be undone.</p>
              <div className="flex gap-3">
                <button onClick={() => setShowDeleteConfirm(false)} className="flex-1 h-[42px] bg-transparent text-secondary hover:bg-surface-container-low transition-colors font-bold text-sm rounded">Cancel</button>
                <button onClick={handleDelete} disabled={deleteMutation.isPending} className="flex-1 h-[42px] bg-error text-white hover:bg-error/90 transition-all font-bold text-sm rounded shadow-md disabled:opacity-40">

...
export default function SimplePurchaseDetailPage() {
  ...
  const { data: purchase, isLoading, error } = useQuery<PurchaseDetail>({
    queryKey: ["simple-purchase", id],
    queryFn: async () => {
      const res = await fetch(`/api/simple/purchases/${id}`);
  ...
  const paymentMutation = useMutation({
    mutationFn: async (data: { amount: number; account_id: string; payment_date: string; note?: string }) => {
      const res = await fetch(`/api/simple/purchases/${id}/payments`, {
        method: "POST",
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["simple-purchase", id] });
  ...
  return (
    <div className="p-4 md:p-8">
      <Breadcrumb items={[{ label: 'Dashboard', href: '/' }, { label: 'Simple Purchases', href: '/purchases-simple' }, { label: `Purchase #${id.slice(0,4).toUpperCase()}`, href: null }]} />
      {/* Back + Title + Actions */}
      <div className="flex flex-col md:flex-row md:items-center gap-3 mb-6">
        <div className="flex items-center gap-3">
          ...
          <h1 className="font-display text-lg md:text-xl font-bold text-primary-container">
            Purchase #{id.slice(0, 4).toUpperCase()}
          </h1>
      ...
      <div className="md:hidden bg-surface-container-lowest p-5 rounded-xl shadow-sm border border-outline-variant mb-6">
        <h2 className="font-display text-lg font-semibold text-on-surface mb-4">Payment Summary</h2>
        <div className="space-y-3">
          <div className="flex justify-between items-center">
            <span className="text-on-surface-variant text-sm">Total Amount</span>
            <span className="font-mono font-medium text-on-surface">{formatMoney(purchase.total_amount)}</span>
          </div>
          <div className="flex justify-between items-center">
            <span className="text-tertiary text-sm">Total Paid</span>
            <span className="font-mono font-bold text-tertiary">{formatMoney(purchase.paid_amount)}</span>
          </div>
          <div className="flex justify-between items-center">
            <span className="text-warning text-sm">Balance Due</span>
            <span className="font-mono font-bold text-warning">{formatMoney(purchase.due_amount)}</span>
          </div>
          <div className="pt-2">
              ...
              {purchase.items.map((item) => (
                ...
                >
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <span className="font-mono text-on-surface-variant text-sm font-bold">
                      {formatMoney(item.total_amount)}
                    </span>
                  </div>
      ...
      <div className="md:hidden fixed bottom-0 left-0 w-full bg-white/90 backdrop-blur-md border-t border-outline-variant px-4 py-3 z-40 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
        <button
          onClick={openPaymentModal}
          disabled={purchase.status === "paid"}
          className="w-full h-12 bg-primary-container text-white font-bold rounded-lg flex items-center justify-center gap-2 active:scale-95 transition-all shadow-lg disabled:opacity-40"
        >
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-primary-container/40 backdrop-blur-sm p-4">
          ...
          <div className="bg-white w-full max-w-sm rounded-xl shadow-lg border border-outline-variant/30 relative z-10 p-6">
            <h2 className="font-display font-bold text-xl text-primary-container mb-2">
              Delete Purchase
            </h2>
            ...
            {deleteError && (
              <p className="text-sm text-error font-medium mb-4">{deleteError}</p>
            )}
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex-1 h-[42px] bg-error text-white hover:bg-error/90 transition-all active:scale-95 font-bold text-sm rounded shadow-md disabled:opacity-40 flex items-center justify-center"
              >

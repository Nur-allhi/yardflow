...
export default function NewSimplePurchasePage() {
  ...
  const mutation = useMutation({
    mutationFn: async (formData: Record<string, unknown>) => {
      const res = await fetch("/api/simple/purchases", {
        method: "POST",
    ...
    onSuccess: (result) => {
      ...
      router.push(`/purchases-simple/${result.id}`);
    },
    onError: (err: Error) => {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    const validItems = items.filter(
      (item) => item.description.trim() && item.quantity_kg && item.price_per_kg,
    );
  ...
  return (
    <div className="p-4 md:p-8">
      <Breadcrumb items={[{ label: 'Dashboard', href: '/' }, { label: 'Simple Purchases', href: '/purchases-simple' }, { label: 'New Purchase', href: null }]} />
      {/* Back + Title */}
            ...
            <section className="bg-white rounded-lg border border-outline-variant/50 shadow-sm p-5 md:p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div className="space-y-1.5">
                  ...
                  {selectedVendor && selectedVendor.due_balance > 0 && (
                    <p className="text-xs text-warning flex items-center gap-1 font-medium">
                      <span className="material-symbols-outlined text-[14px]">
                        account_balance_wallet
                      </span>
                      Vendor due balance: {formatMoney(selectedVendor.due_balance)}
                    </p>
                  )}
                </div>
                <div className="space-y-1.5">
            ...
            <section className="bg-white rounded-lg border border-outline-variant/50 shadow-sm overflow-hidden">
                        ...
                        <td className="px-6 py-3">
                          <div className="relative">
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                handleItemChange(item.key, "price_per_kg", e.target.value)
                              }
                              autoComplete="off"
                              inputMode="decimal"
                              enterKeyHint="next"
                        ...
                        <td className="px-6 py-3 text-center">
                          {items.length > 1 && (
                            <button
                              ...
                              className="text-error hover:bg-error/5 p-1 rounded transition-colors"
                            >
              ...
              <div className="md:hidden space-y-3 p-4">
                {items.map((item) => (
                  <div
                    key={item.key}
                    className="border border-outline-variant/50 rounded-lg p-3 space-y-3"
                  >
                    <input
                      type="text"
                      value={item.description}
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="text-[10px] uppercase font-bold text-secondary block mb-1">
                          Qty (kg)
                        </label>
                        <input
                          type="number"
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      inputMode="decimal"
                      enterKeyHint="next"
                      className="w-full h-[44px] pl-8 pr-3 border border-outline-variant rounded text-sm font-mono focus:border-primary-container focus:ring-0 outline-none transition-all"
                      placeholder="0.00"
                    />
                  </div>
                </div>
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-mono font-bold">
                    {items.filter((i) => i.description.trim() && i.quantity_kg && i.price_per_kg).length}
                  </span>
                </div>
                <div className="flex justify-between items-center">
                ...
                {paid > 0 && (
                  <div className="flex justify-between items-center">
                    <span className="text-white/70 text-sm">Paying Now</span>
                ...
                {paid > 0 && (
                  <div className="flex justify-between items-center border-t border-white/20 pt-3">
                    <span className="text-white/80 font-semibold">Due After Payment</span>
                    <span className="font-mono font-bold text-warning">
                      {formatMoney(Math.max(0, totalAmount - paid))}
                    </span>
                  </div>
                )}
              </div>
        ...
        <div className="md:hidden fixed bottom-0 left-0 w-full bg-white/90 backdrop-blur-md border-t border-outline-variant px-4 py-3 z-40 flex items-center justify-between shadow-[0_-4px_12px_rgba(0,0,0,0.05)]">
          <div className="flex flex-col">
            <span className="text-[10px] text-on-surface-variant font-bold uppercase tracking-tight">
              {items.filter((i) => i.description.trim() && i.quantity_kg && i.price_per_kg).length} Items
            </span>

...
export default function CustomerProfilePage() {
  ...
  async function handlePayment(e: React.FormEvent) {
    ...
    if (!paySaleId) {
      setPayError("Please select a sale to receive payment against");
      return;
    }
  ...
  const dueSales = customer.sales.filter((s) => s.status !== "paid");

  return (
    <div className="p-4 md:p-8">
      <Breadcrumb items={[{ label: 'Dashboard', href: '/' }, { label: 'Sales', href: '/sales' }, { label: 'Customers', href: '/sales/customers' }, { label: 'Customer Profile', href: null }]} />
      {/* Back + Title */}
                  ...
                  <tbody className="divide-y divide-outline-variant/30">
                    {customer.sales.map((s) => (
                      <tr key={s.id} className="hover:bg-surface-bright">
                        ...
                        <td className="px-6 py-4">
                          <TypeChip type={s.sale_type} isQuickCash={s.is_quick_cash_sale} />
                        </td>
          ...
          <div className="bg-primary-container text-white rounded-xl p-6 shadow-md relative overflow-hidden">
            ...
            <div className="space-y-4 relative z-10">
              <div className="flex justify-between items-center">
                ...
                <span className="font-mono font-bold text-white">{formatMoney(customer.opening_balance)}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-white/70 text-sm">Total Sales</span>
                <span className="font-mono font-bold text-white">{formatMoney(customer.summary.total_sale_amount)}</span>
              </div>
              <div className="flex justify-between items-center">
